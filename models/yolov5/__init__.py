...
def YoloNet5(ch_in, dm=None, wm=None, anchors=None):
    cfg = str(pathlib.Path(__file__).parent.resolve()) + '/models/yolov5n.yaml'
    cfg = load(cfg, dm, wm)
    return YoloNet(cfg, ch_in, 1, anchors=anchors)

def YoloNet8(ch_in, dm=None, wm=None, anchors=None):
    cfg = str(pathlib.Path(__file__).parent.resolve()) + '/models/yolov8n.yaml'
    cfg = load(cfg, dm, wm)
    return YoloNet(cfg, ch_in, 1, anchors=anchors)

def YoloNetPhi(ch_in, dm=None, wm=None, a=0.35, b0=7, b=1, se=False, c2=True, anchors=None):
